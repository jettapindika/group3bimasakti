  ...
  return (
    <div ref={containerRef} className="relative w-full h-full overflow-hidden rounded-2xl">
      {imageUrl && (
        ...
        >
          <img src={imageUrl || "/placeholder.svg"} alt="Reveal" className="w-full h-full object-cover" />
        </motion.div>
      )}

      <svg className="absolute inset-0 w-full h-full" style={{ mixBlendMode: "screen", pointerEvents: "none" }}>
        <defs>
          <radialGradient id="revealGradient" cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor="rgba(212, 166, 42, 0.9)" />
            <stop offset="100%" stopColor="rgba(212, 166, 42, 0)" />
          </radialGradient>
